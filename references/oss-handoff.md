...
用户请求 `compile` 模板包时已经授权完成模板生产所必需的 OSS 上传。`produce` 先完成候选视觉审批；数量目标或“模板包”字样不能跳过审批，批准本身也不触发上传。只有批准后再次收到“产出通过的模板包”“完成 OSS 最终化”或“上传 OSS”等明确指令，才授权 OSS 外部写入。用户要求“仅预览”或“暂不上传”时停在 `prepublish`，不创建 `package/`。完整真图评测不构成 OSS 最终化的前置条件。
