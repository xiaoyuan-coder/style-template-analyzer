...
5. 按语义完整性检查裁切：独立 `artwork` 和 `emblem` 默认保留完整外轮廓与约 8%–12% 安全边距；分镜景别、满版构图和抽象关系带可以主动触边，只要载体逻辑清楚，主体身份、核心识别特征与关键关系仍可读。普通印制模板使用纯图案结构；外框、放大镜小窗、连接线和分析标注只在用户明确要求图鉴、档案、分析或说明书效果时启用。
